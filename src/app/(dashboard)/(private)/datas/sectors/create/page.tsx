...
const CreateSectorPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Sektör oluşturulurken hata oluştu:', error);
      toast.error(error.response?.data?.error?.message || 'Sektör oluşturulurken bir hata oluştu');
    } finally {
      setLoading(false);
    }
  };
          ...
          <Grid container spacing={4}>
            ...
            <Grid item xs={12}>
              <Button
                ...
                disabled={loading}
              >
                {loading ? 'Sektör Oluşturuluyor...' : 'Sektörü Oluştur'}
              </Button>
            </Grid>
          </Grid>
        </form>
